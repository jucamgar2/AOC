...
public class Day12Solution2024 extends BaseDay{

    @Override
    protected Day12Y2024 getInputData(){
        return Day12Y2024.getDay12Data();
    }
    
    @Override
    public void runDaySolution(){
        Day12Y2024 input = getInputData();
    ...
    protected void part1Solution(Object input) {
        System.out.println("SOLUCION: " + getPriceOfFencing((Day12Y2024)input));
    }

    private static List<Set<Position>> getRegions(Day12Y2024 input){
    ...
    private static void addToRegion(Position p, Day12Y2024 input, Set<Position> region, List<Position> visited){
    ...
    private static Long getPriceOfFencing(Day12Y2024 input){
    ...
    private static Long getPerimeter(Set<Position> region, Day12Y2024 input) {
        return region.stream().mapToLong(flower->getDifferentFlowersArround(flower, input)).sum();
    }

    private static long getArea(Set<Position> region, Day12Y2024 input) {
        return region.size();
    }

    private static long getDifferentFlowersArround(Position flower, Day12Y2024 input){
        int i = flower.getI();
        int j = flower.getJ();
    ...
    protected void part2Solution(Object input) {
        System.out.println("SOLUCION: " + getPriceOfFencingPart2((Day12Y2024)input));
    }

    private static Long getPriceOfFencingPart2(Day12Y2024 input){
        List<Set<Position>> regions = getRegions(input);
        return regions.stream().mapToLong(region->getArea(region, input)*getRegionSides(region, input)).sum();
    }

    private static long getRegionSides(Set<Position> region, Day12Y2024 input) {
        int sides = 0;
    ...
    private static int countVertices(Position p, Day12Y2024 input){
        int i = p.getI();
        int j = p.getJ();
