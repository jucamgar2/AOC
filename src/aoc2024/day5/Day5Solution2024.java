...
public class Day5Solution2024 extends BaseDay{

    @Override
    protected Day5Y2024 getInputData(){
        return Day5Y2024.getDay5Data();
    }

    @Override
    public void runDaySolution(){
        Day5Y2024 input = getInputData();
    ...
    @Override
    public void part1Solution(Object input){
        Day5Y2024 parsedInput = (Day5Y2024) input;
        Integer res = parsedInput.getListOfUpdates().stream().filter(list->isUpdateListValid(list, parsedInput)).mapToInt(list->list.get(list.size()/2)).sum();
        System.out.println("SOLUCIÓN: " + res);
    }

    public static boolean isUpdateListValid(List<Integer> listOfupdates, Day5Y2024 input){
    ...
    @Override
    public void part2Solution(Object input){
        Day5Y2024 parsedInput = (Day5Y2024) input;
        Integer res = parsedInput.getListOfUpdates().stream().filter(list->!isUpdateListValid(list, parsedInput))
            .map(list->orderLists(list,parsedInput))
            .mapToInt(list->list.get(list.size()/2)).sum();
        System.out.println("SOLUCIÓN: " + res);    
    }

    public static List<Integer> orderLists(List<Integer> listToOrder, Day5Y2024 input){
